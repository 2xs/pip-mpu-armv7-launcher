...
if __name__ == '__main__':
    if len(sys.argv) >= 4:
        ...
        data_size = xs[2]
        bss_size = xs[3]
        print(f'set $flash_base = $flash_end')
        print(f'set $ram_base = $ram_end')
        ...
        print(f'add-symbol-file {soft_path} '
               ...
               '-s .rom.ram $rel_data '
               '-s .ram $bss')
        print(f'set $flash_end = $flash_base + {crt0_meta_size + text_size + got_size + data_size}')
        print(f'set $ram_end = $ram_base + {got_size + data_size + bss_size}')
        sys.exit(0)
    usage()
